...
##################
if [ $MGR_USER ] && [ $MGR_PASSWORD ]; then
    /opt/jboss/keycloak/bin/add-user.sh $MGR_USER $MGR_PASSWORD
fi
...
# REST API #
############
if [ "$DEV_PORTAL_URL" ]; then
    SYS_PROPS+=" -Ddev.portal.url=$DEV_PORTAL_URL"
fi

if [ "$APP_PORTAL_URL" ]; then
    SYS_PROPS+=" -Dapp.portal.url=$APP_PORTAL_URL"
fi
